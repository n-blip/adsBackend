...
$query = sprintf("SELECT address, city, state, zipcode, latitude, longitude, ( 3959 * acos( cos( radians('%s') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( latitude ) ) ) ) AS distance FROM denny_locations HAVING distance < '%s' ORDER BY distance LIMIT 0 , %s",
